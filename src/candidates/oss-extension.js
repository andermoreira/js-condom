...
} from './oss-baseline.js';

export const CANDIDATE_ID = 'oss-extension';
...
export class OssExtensionError extends Error {
  constructor(code, message, details = {}) {
    super(message);
    this.name = 'OssExtensionError';
    this.code = code;
...
export function protect({
  ...
  try {
    ...
    throw new OssExtensionError(error.code ?? 'invalid_canonical_seed', error.message, error.details);
  }

  try {
    assertAuxiliaryTransformsAllowed(auxiliaryTransforms);
  } catch (error) {
    throw new OssExtensionError(error.code, error.message, error.details);
  }
