...
import {
  ...
  OssExtensionError,
  protect,
} from '../../src/candidates/oss-extension.js';
import {
...
test('rejects auxiliary transforms outside the slice allowlist', () => {
  assert.throws(
    ...
    (error) =>
      error instanceof OssExtensionError && error.code === 'invalid_auxiliary_transform',
  );
});
...
test('rejects empty input with diagnostics', () => {
  assert.throws(
    ...
    (error) => error instanceof OssExtensionError && error.code === 'invalid_input',
  );
});
